...
class _SelfProfileStartUpState extends State<SelfProfileStartUp> {
  ...
  getUser() async {
    QuerySnapshot temp = await dataBaseMethods.getUserbyemail(widget.email, "startup");
                  ...
                  onPressed: () {
                    if (index == 0) {
                      ...
                      if (headquartersController.text == '') {
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please enter a headquarters'),),
                        );
                        return;
                      }
                      ...
                      setState(() {
                        searchListStartup.headquarters = headquartersController.text;
                      });
                      Navigator.pop(context);
                    }
                  },
  ...
  Future pickImage(BuildContext context) async {
    try {
      ...
      if (!mounted) return;
      await dataBaseMethods.uploadLogo(context,image: _image, email: widget.email, type: "startups");
      await dataBaseMethods.getAllLogos("startups").then((value) {
        setState(() {
          downloadUrls = value;
          url = downloadUrls["${widget.email}_photo"] ?? "";
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        Text(searchListStartup.name!, style: const TextStyle(fontSize: 20.0, fontWeight: FontWeight.bold,),),
                        const SizedBox(height: 10.0),
                      ],
                    ),
                  ),
                    ...
                    child: Column(
                      children: [
                        ...
                        Padding(
                          ...
                          child: Align(
                              alignment: Alignment.centerLeft,
                              child: Text(searchListStartup.sector!, style: const TextStyle(color: Color.fromARGB(255, 117, 116, 116)))
                          ),
                        ),
                        const SizedBox(height: 10.0),
                      ],
                    ...
                    child: Column(
                      children: [
                        ...
                        Padding(
                          ...
                          child: Align(
                              alignment: Alignment.centerLeft,
                              child: Text(searchListStartup.headquarters!, style: const TextStyle(color: Color.fromARGB(255, 117, 116, 116)),)
                          ),
                        ),
                        const SizedBox(height: 10.0),
                      ],
