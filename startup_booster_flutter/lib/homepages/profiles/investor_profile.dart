...
class _InvestorProfileState extends State<InvestorProfile> {
  ...
  getToken() async {
    token = await dataBaseMethods.getUserTokenbyEmail(widget.investorModel.email!, "startup");
  }
  ...
  void initState() {
    ...
    getToken();
    checkIfFriend();
    dataBaseMethods.getFriends('investor', widget.investorModel.email!).then((value) {
      setState(() {
  ...
  checkIfFriend() async {
    isFriend = await dataBaseMethods.isFriend("startup", currentEmail: widget.email, email: widget.investorModel.email!);
    setState(() {
      isRequested = false;
    });
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          isRequested ? const CircularProgressIndicator() : !isFriend ? IconButton(
                            ...
                            builder: (context) {
                              return AlertDialog(
                                  ...
                                  children: [
                                    TextField(
                                      ...
                                      decoration: const InputDecoration(
                                        hintText: "Enter invested amount in millions",
                                      ),
                                    ),
                                ...
                                actions: [
                                  ...
                                  TextButton(
                                    onPressed: () {
                                      dataBaseMethods.addFriendRequest("startup", context,
                      ...
                      icon: Image.asset("assets/images/add-friend.png", width: 25, height: 25),
                    ) : IconButton(icon: Image.asset("assets/images/remove-friend.png"),
                      onPressed: () {
                        showDialog(
                            context: context,
                            builder: (context) {
                                  ...
                                  TextButton(
                                    onPressed: () {
                                      dataBaseMethods.removeInvestorAsFriend(
                                          email: widget.email,
                                          currentEmail: widget.investorModel.email!);
                                      Navigator.pop(context);
                                    },
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Padding(
                                padding: const EdgeInsets.only(left: 20.0),
                                child: ReadMoreText("• ${widget.investorModel.aboutCompany!}",
                                  trimLines: 2,
                  ...
                  StreamBuilder(
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        ...
                        return const Center(child: Text("No associations"),);
                      } else {
