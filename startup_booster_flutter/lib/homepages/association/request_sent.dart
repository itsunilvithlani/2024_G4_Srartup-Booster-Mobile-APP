...
class _RequestSentState extends State<RequestSent> {
  ...
  getPendingRequest() async {
    pendingRequestStream = await dataBaseMethods.getFriendRequests(widget.type, widget.email);

//    await dataBaseMethods.getAllLogos(widget.type == "investor" ? "startups" : "investors").then((value) {downloadUrls = value;});
    if (mounted) {
      setState(() {
        isLoading = false;
      });
  ...
  Widget listView(list) {
    return ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return list.docs[index]['sender'] == widget.email ? Container(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Row(
                            children: [
                              ClipOval(child: downloadUrls['${url}_photo'] == null ? const Icon(Icons.account_circle, size: 50) : Image.network(
                              ...
                              SizedBox(
                                ...
                                child: Text(list.docs[index]['name'], style: const TextStyle(fontSize: 16.0, fontWeight: FontWeight.bold,),),
                              ),
                            ],
                          ),
                          TextButton(
    ...
    return isLoading ? const Center(
            ...
            builder: ((context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                return checkLength ? listView(snapshot.data) : const Center(child: Text("No requests sent"));
              } else {
                return Container();
              }
            }));
